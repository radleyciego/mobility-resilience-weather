...
def add_lagged_features(df: pl.DataFrame, lag_hours: list[int] | None = None) -> pl.DataFrame:
    lags = lag_hours or [1, 2, 3, 24]
    df = df.sort(["PULocationID", "pickup_datetime"])
    for lag in lags:
        df = df.with_columns(
            pl.col("trip_count")
            .shift(lag)
            .over("PULocationID")
            .alias(f"trip_count_lag{lag}h")
        )
    return df
